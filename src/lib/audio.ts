...
export const mulawDecodeByte = (ulaw: number): number => {
  ulaw = ~ulaw & 0xff;
...
export const mulawEncodeSample = (sample: number): number => {
  let sign = 0;
  if (sample < 0) {
    sample = -sample;
    sign = 0x80;
...
export const mulawDecode = (mulaw: Buffer): Buffer => {
  const out = Buffer.alloc(mulaw.length * 2);
  for (let i = 0; i < mulaw.length; i++) {
    out.writeInt16LE(mulawDecodeByte(mulaw[i]), i * 2);
  }
  return out;
};

export const mulawEncode = (pcm: Buffer): Buffer => {
...
export const upsample8to16 = (pcm8k: Buffer): Buffer => {
...
export const downsample24to8 = (pcm24k: Buffer): Buffer => {
...
export const chunkBuffer = (buf: Buffer, chunkSize: number): Buffer[] => {
  const chunks: Buffer[] = [];
