...
const tokenPath = (): string =>
  path.resolve(process.cwd(), config.GMAIL_TOKEN_PATH);

export const makeOAuthClient = (): Auth.OAuth2Client => {
...
export const saveToken = async (token: Auth.Credentials): Promise<void> => {
  const p = tokenPath();
...
export const loadToken = async (): Promise<Auth.Credentials | null> => {
  try {
...
export const getAuthorizedClient = async (): Promise<Auth.OAuth2Client> => {
  ...
  client.on("tokens", (next) => {
    saveToken({ ...token, ...next }).catch((err) =>
      Logger.warn("[gmail] failed to persist refreshed token", err)
    );
  });
  return client;
};
